...
class EditParabasi extends StatefulWidget {
  const EditParabasi({
  ...
  @override
  _EditParabasiState createState() => _EditParabasiState();
}

class _EditParabasiState extends State<EditParabasi> {
  late List<Map<String, dynamic>> _data = [];
  quill.QuillController _quillController = quill.QuillController.basic();
  TextEditingController _imerominiaParabasisController =
  ...
  Future<void> _getParabasi() async {
    final results = await widget.database.rawQuery(
        'SELECT * FROM parabaseis WHERE id = ${widget.numberParabasi}');
    setState(() {
      _data = results;
      _imerominiaParabasisController = TextEditingController(
          text: DateFormat('dd/MM/yyyy')
              .format(DateTime.fromMillisecondsSinceEpoch(_data[0]['date'])));
      epilogiParabasis = _data[0]['parabasi'];
      selectedArticle = _data[0]['article'];
      _afairesiDiplomatos = _data[0]['diploma'] == 1 ? true : false;
      _imeresDiplomatos = _data[0]['imeres_dipl'];
      _afairesiStoixeion = _data[0]['stoixeia'] == 1 ? true : false;
      _imeresStoixeion = _data[0]['imeres_stoixeion'];
      keimeno = _data[0]['keimeno'];
      List<Map<String, dynamic>> textList =
          List<Map<String, dynamic>>.from(jsonDecode(keimeno));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Επεξεργασία Παράβασης'),
        centerTitle: true,
        leading: MouseRegion(
              ...
              ElevatedButton(
                onPressed: () async {
                  ...
                  await widget.database.transaction((txn) async {
                    await txn.rawUpdate(
                        'UPDATE parabaseis SET date = ?, parabasi = ?, article = ?, diploma = ?, imeres_dipl = ?, stoixeia = ?, imeres_stoixeion = ?, keimeno = ? WHERE id = ?',
                        [
                          imerominiaParabasis,
                          parabasi,
                          article,
